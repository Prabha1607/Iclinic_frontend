...
import { initiateCall } from '../features/bookAppointment/patient-bookAppointment/services/bookingService';

type Step = 'idle' | 'callForm' | 'loading' | 'success';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (
        ...
        msg = (err as any).response.data.detail;
      }
  ...
  return (
    <>
      <style>{`
        ...
        .widget-bounce { animation: widgetBounce 2.8s ease-in-out infinite; }
        @keyframes widgetBounce { 0%,100%{transform:translateY(0);} 50%{transform:translateY(-5px);} }
        .widget-ring { position:absolute; inset:-4px; border-radius:9999px; border:2px solid #3b5bfc; animation:ringPulse 2.5s ease-out infinite; pointer-events:none; }
        .widget-ring-2 { animation-delay:0.8s; inset:-8px; opacity:0.4; }
        @keyframes ringPulse { 0%{opacity:0.6;transform:scale(1);} 100%{opacity:0;transform:scale(1.35);} }
        .panel-enter { animation:panelEnter 0.28s cubic-bezier(0.34,1.56,0.64,1) forwards; transform-origin:bottom right; }
        @keyframes panelEnter { from{opacity:0;transform:scale(0.8) translateY(10px);} to{opacity:1;transform:scale(1) translateY(0);} }
        .widget-tooltip { animation:tooltipSlide 0.2s ease forwards; }
        @keyframes tooltipSlide { from{opacity:0;transform:translateX(8px);} to{opacity:1;transform:translateX(0);} }
        .success-check { animation:checkPop 0.4s cubic-bezier(0.34,1.56,0.64,1) forwards; }
        @keyframes checkPop { from{transform:scale(0);opacity:0;} to{transform:scale(1);opacity:1;} }
        .loading-dots span { display:inline-block;width:5px;height:5px;border-radius:50%;background:#3b5bfc;animation:dotBounce 1.2s ease-in-out infinite; }
      ...
      <div className="fixed bottom-6 right-6 z-[100] flex flex-col items-end gap-3">
        {(step === 'callForm' || step === 'loading' || step === 'success') && (
          <div
            className="panel-enter bg-white rounded-2xl shadow-2xl border border-blue-100 w-80 overflow-hidden"
          ...
          >
            ...
            {step === 'success' ? (
              <div className="p-6 text-center">
                ...
                <p
                  className="font-bold text-[#0f1340] text-base mb-1"
              ...
              <div className="p-6 text-center">
                <div className="w-12 h-12 rounded-full bg-[#eef2ff] flex items-center justify-center mx-auto mb-4">
                  <svg
                    className="w-5 h-5 text-[#3b5bfc] animate-spin"
                    fill="none"
                    viewBox="0 0 24 24"
                  >
                    <circle
                ...
                <p className="font-semibold text-[#0f1340] text-sm mb-3">
                  Connecting you now…
                </p>
              ...
              <div className="p-5">
                <div className="flex items-center gap-2 mb-4">
                  <div className="flex-1">
                    <p
                      className="font-bold text-[#0f1340] text-sm"
                      style={{ fontFamily: "'Syne', sans-serif" }}
                    >
                      Request a Call Back
                    </p>

  ...
  return (
    <div className="flex flex-col gap-0.5">
      ...
      <span className="text-sm text-slate-700 font-medium">
        {isEmpty ? <span className="text-slate-300 font-normal italic">—</span> : value}
      </span>
    </div>
  );
}
                ...
                <div className="flex items-center gap-3 mb-3 pb-3 border-b border-slate-200">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400">
                      {appointment.user.email || appointment.patient_name}
                    </p>
                  </div>
                </div>
              ) : null}
                ...
                <DetailRow
                  ...
                  value={
                    appointment.user?.phone_no
                      ? `${appointment.user.country_code ?? ''} ${appointment.user.phone_no}`.trim()
                      : null
                  }
                />
                <DetailRow label="Email" value={appointment.user?.email || null} />
                ...
                <>
                  <div className="flex items-center gap-3 mb-3 pb-3 border-b border-slate-200">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-400">
                        {appointment.provider.provider_profile?.specialization ??
                          'General'}
                      </p>
                    </div>
                  </div>
...
export default function FrontDeskAppointments() {
  ...
  const [_doctorSearch, setDoctorSearch] = useState(''); // unused → prefixed with _
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="bg-white rounded-2xl border border-blue-50 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            <div className="w-16 h-16 rounded-3xl bg-[#eef2ff] flex items-center justify-center text-3xl mb-4">
              📅
            </div>
            <p
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-slate-50">
                  {filtered.map((appt) => {
                    ...
                    const ch = appt.booking_channel
                      ? CHANNEL_CONFIG[appt.booking_channel]
                      : null;

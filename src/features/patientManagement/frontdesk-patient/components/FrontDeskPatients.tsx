  ...
  const validate = (f = form): FieldErrors =>
    collectErrors({
      ...
      email:
        ...
        rules.email(f.email) ||
        rules.maxLen(f.email, 100, 'Email'),
      ...
      >
        ...
        <div className="flex items-center justify-between px-7 pt-6 pb-5 border-b border-slate-100 flex-shrink-0">
          <div className="flex items-center gap-4">
            <div
              className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${avatarColor(patient.id)} flex items-center justify-center text-white font-bold text-base flex-shrink-0`}
            >
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="col-span-2">
                <label className="block text-xs font-bold uppercase tracking-widest text-[#0f1340] mb-1.5">
                  New Password{' '}
                  <span className="text-slate-300 font-normal normal-case tracking-normal text-[11px]">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-slate-50 rounded-xl px-4 py-3 border border-slate-100">
              ...
              <p className="text-sm font-semibold text-slate-600">
                {patient.country_code} {patient.phone_no}
              </p>
            </div>
            <div className="bg-slate-50 rounded-xl px-4 py-3 border border-slate-100">
              <p className="text-[10px] font-bold uppercase tracking-widest text-[#3b5bfc] mb-1">
              ...
              <span
                className={`inline-flex items-center gap-1.5 text-xs font-semibold ${patient.is_active ? 'text-emerald-600' : 'text-slate-400'}`}
              >
                <span
                  className={`w-1.5 h-1.5 rounded-full ${patient.is_active ? 'bg-emerald-500' : 'bg-slate-400'}`}
                />
...
export default function FrontDeskPatients() {
  ...
  const fetchPatients = useCallback(async () => {
    ...
    try {
      ...
      setFetchError('Failed to fetch patients. Please try again.');
    } finally {
      setLoading(false);
    }

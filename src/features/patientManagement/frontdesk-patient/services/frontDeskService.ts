...
export const getAppointments = (filters: AppointmentFilters = {}): Promise<Appointment[]> => {
  ...
  return api.get<Appointment[]>('/api/v1/booking/list', { params }).then((res) => res.data)
}
...
export const getPatients = (filters: PatientFilters = {}): Promise<Patient[]> => {
  const params: Record<string, string | number | boolean> = {}
  if (filters.page) params.page = filters.page
  if (filters.page_size) params.page_size = filters.page_size
  if (filters.is_active != null) params.is_active = filters.is_active

  return api.get<Patient[]>('/api/v1/users/list', { params }).then((res) => res.data)
}
...
export const updatePatient = (user_id: number, data: PatientUpdatePayload): Promise<Patient> =>

...
import { getUserAppointments } from '../../../bookAppointment/patient-bookAppointment/services/bookingService';
      ...
      <div className="p-5">
        ...
        {appt.provider && (
          <div className="flex items-center gap-2 mb-3">
            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-emerald-400 to-teal-500 flex items-center justify-center text-white text-xs font-bold flex-shrink-0">
              {`${appt.provider.first_name[0]}${appt.provider.last_name[0]}`.toUpperCase()}
            </div>
            <div>
              ...
              <p className="text-xs font-semibold text-emerald-600">
                {appt.provider.provider_profile?.specialization ?? 'Doctor'}
              </p>
            </div>
          </div>
        )}
        ...
        {appt.appointment_type && (
          <p className="text-xs text-slate-500 mb-2">
            📋 {appt.appointment_type.name} · {appt.appointment_type.duration_minutes} min
          </p>
        )}
...
export default function PatientDashboard() {
  ...
  const [loading, setLoading] = useState(true); // start true — always attempt fetch
  const [error, setError] = useState('');
  const [filter, setFilter] = useState<FilterStatus>('ALL');

  useEffect(() => {
    // userId comes from Redux which is populated by RequireAuth's silent refresh.
    // If still null here, wait — the effect re-runs when userId changes.
  ...
  }, [userId]); // re-fetch whenever userId becomes available
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ fontFamily: "'DM Sans', sans-serif", background: '#f8faff' }}>
      ...
      <main className="flex-1 max-w-6xl mx-auto w-full px-4 sm:px-6 py-8">
        ...
        {/* userId not yet available — still restoring session */}
        {userId === null ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <button
              onClick={() => { setError(''); setLoading(true); getUserAppointments(userId!).then(setAppointments).catch(() => setError('Unable to load appointments.')).finally(() => setLoading(false)); }}
              className="mt-4 px-4 py-2 bg-[#3b5bfc] text-white text-sm font-medium rounded-xl hover:bg-[#2f4edc] transition"
            >
              Try Again
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {sorted.map((appt) => <AppointmentCard key={appt.id} appt={appt} />)}
          </div>
        )}
      </main>
    </div>
